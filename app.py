...
if uploaded_file is not None:
    ...
    # Prediction
    prediction = model.predict(img_array)[0]  # Example: [0.85]
    prob_tumor = float(prediction[0])
    prob_no_tumor = 1 - prob_tumor

    # Final result
    result = "🧠 Tumor Detected" if prob_tumor > 0.5 else "✅ No Tumor"
    ...
    fig, ax = plt.subplots()
    bars = ax.bar(class_names, [prob_no_tumor, prob_tumor], color=["green", "red"])
